...
class LandmarkDetailTests: XCTestCase {
    func testViewHasExpectedViews() throws {
        ...
        let exp = ViewHosting.loadView(LandmarkDetail(landmark: landmark), environmentObject: appModel).inspection.inspect { view in
            let mapView = XCTAssertNoThrowAndAssign(try view.find(MapView.self))
            ...
            let circleImage = XCTAssertNoThrowAndAssign(try view.find(CircleImage.self))
            XCTAssertEqual(try circleImage?.actualView().image, landmark.image)

            let landmarkName = XCTAssertNoThrowAndAssign(try view.find(ViewType.Text.self, index: 0))
            ...
            let landmarkPark = XCTAssertNoThrowAndAssign(try view.find(ViewType.Text.self, index: 1))
            XCTAssertEqual(try landmarkPark?.string(), landmark.park)
            let landmarkState = XCTAssertNoThrowAndAssign(try view.find(ViewType.Text.self, index: 2))
            XCTAssertEqual(try landmarkState?.string(), landmark.state)
            let landmarkAbout = XCTAssertNoThrowAndAssign(try view.find(ViewType.Text.self, index: 3))
            XCTAssertEqual(try landmarkAbout?.string(), "About \(landmark.name)")
            let landmarkDescription = XCTAssertNoThrowAndAssign(try view.find(ViewType.Text.self, index: 4))
