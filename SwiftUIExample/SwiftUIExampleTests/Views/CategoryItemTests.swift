...
class CategoryItemTests: XCTestCase {
    ...
    func testCategoryRowDisplaysCategoryNameWithLandmarks() throws {
        let data = try JSONSerialization.data(withJSONObject: Self.landmark)
        let landmark =  try JSONDecoder().decode([Landmark].self, from: data).first!
...
extension CategoryItemTests {
    ...
    static let landmark: [[String: Any]] = [[
        "name": "Chilkoot Trail",
        "category": "Mountains",
        "city": "Skagway",
        "state": "Alaska",
