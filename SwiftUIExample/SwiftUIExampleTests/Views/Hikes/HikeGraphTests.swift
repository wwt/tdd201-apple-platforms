...
class HikeGraphTests: XCTestCase {
    ...
    func testHighGraphSetsColorBasedOnPath() throws {
        ...
        try hikeGraph.inspect().geometryReader(0).hStack().forEach(0).enumerated().forEach {
            XCTAssertEqual(try $0.element.find(GraphCapsule.self).colorMultiply(), Color(hue: 0, saturation: 0.5, brightness: 0.7))
        }

        hikeGraph = HikeGraph(hike: expectedHike, path: \.pace)
        try hikeGraph.inspect().geometryReader(0).hStack().forEach(0).enumerated().forEach {
            XCTAssertEqual(try $0.element.find(GraphCapsule.self).colorMultiply(), Color(hue: 0.7, saturation: 0.4, brightness: 0.7))
        }

        hikeGraph = HikeGraph(hike: expectedHike, path: \.newRange)
        try hikeGraph.inspect().geometryReader(0).hStack().forEach(0).enumerated().forEach {
            XCTAssertEqual(try $0.element.find(GraphCapsule.self).colorMultiply(), .black)
        }
    }
